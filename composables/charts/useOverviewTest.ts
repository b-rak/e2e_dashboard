...
export default async (
  ...
  if (update) {
    ...
    let chart = Chart.getChart("chart-1") as Chart;
    ...
    (chart.config.data.datasets[0] = {
      ...
      barThickness: breakpoint === "lg" ? useRem() * 4 : useRem() * 5.156,
      minBarLength: 20,
    }),
      (chart.config.options.plugins.datalabels.font.size = useRem() * 0.75);
    console.log("SIZE", chart.config.options.plugins.datalabels.font.size);
    chart.update();
  } else {
    const data = await getResults(dashboardId);
    ...
    // Bar chart
    new Chart(
      document.getElementById("chart-" + groupdId) as HTMLCanvasElement,
      {
        type: "bar",
          ...
          datasets: [
            {
              ...
              categoryPercentage: useRem() * 0.063,
              barThickness:
                breakpoint === "lg" ? useRem() * 4 : useRem() * 5.156,
              minBarLength: 20,
            },
          ],
        },
